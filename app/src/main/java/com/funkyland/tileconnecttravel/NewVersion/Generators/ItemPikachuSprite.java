...
//        GameConfiguration.pathTHEME = "item"+ IPreferences.getInstance(mContext).getTheme()+"/";


        if (Pereferences.get_gameMode().equals(GAME_MODE_CLAS)) {
            GameConfiguration.pathTHEME = "item"+IPreferences.getInstance(mContext).getTheme() + "/";
        } else {
            GameConfiguration.pathTHEME = IPreferences.getInstance(mContext).getThemePath() + "/";
        }

        String s = (GameConfiguration.pathTHEME) + "item" + this.GT_ITEM + ".png";
        Log.d("Kishan", "" + s);
        ...
        if (!ItemsManager.map_Resources.containsKey(this.GT_ITEM)) {
            ...
            this.item_TR = BitmapTextureAtlasTextureRegionFactory.createFromAsset(this.item_BTA, this.mContext, s, 0, 0);
    ...
    public void onLoadScene(final Scene mScene) {
        this.mScene = mScene;
        this.item_SP = new Sprite(-100.0f, -100.0f, (float) GameConfiguration.ITEM_WIDTH,
                (float) GameConfiguration.ITEM_HEIGHT, this.item_TR) {
            @Override
